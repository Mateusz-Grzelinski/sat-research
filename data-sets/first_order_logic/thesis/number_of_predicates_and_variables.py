...
ATOM_CLAUSE_RATIO_PATH = "_fol-tptp-cnf-number-of-predicates-and-variables"
...
def job(system_property_gen: CNFSafetyLivenessPresetNoSolver):
    for i in range(number_of_formula_instances):
        out_file_path = os.path.join(
            ...
            f'predicate_names{len(system_property_gen.predicate_names)}-variables_names{len(system_property_gen.variable_names)}',
            f'{i}'
        )
...
if __name__ == '__main__':
    test_number_of_predicates = [10, 25, 50, 100, 250, 500, 1000]
    test_number_of_variables = [10, 25, 50, 100, 250, 500, 1000]
    ...
    for number_of_predicates in test_number_of_predicates:
        for number_of_variables in test_number_of_variables:
            predicate_names = [f'p{i}' for i in range(number_of_predicates)]
